...
6. UPIS 8.0 재시작 후 지도 정상 표시 여부 확인', 'UPIS 표준시스템 초기 구축 시 사업단 권고 메모리 기준값을 서버 사양에 맞게 검토하고, PermSize(min/max)를 운영 환경에 적합한 값으로 설정한다.', 'UPIS, 표준시스템, Tomcat, 톰캣, GWS, ClientAbortException, OutputBuffer.realWriteBytes, PermSize, 메모리부족, 지도미표시, 장애', 55, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. 생성된 테스트 계정의 정상 작동 여부를 확인한다.', '좌표계 특이 지자체 목록을 사전에 문서화하고, Vworld 보정좌표 미적용 지역에 대한 안내문을 배포하여 반복 문의를 예방한다.', 'UPIS, 표준시스템, 좌표계, 위치오차, Vworld, 보정좌표, 배경지도, 철도라인, TN_USER_NEW, TN_AUTHOR_USER_NEW, 테스트계정, 권한, GSS, GPKI, 고령군, 울릉군, K데이터', 18, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 표준시스템 재기동 후 새올행정 연계 화면이 정상적으로 표출되는지 확인한다.', 'GPKI 서버 인증서 만료 전(권고: 만료 30일 전) 기관 담당자와 갱신 일정을 사전 조율하고, 인증서 경로(GPKI 폴더·gpkisecureweb 폴더)와 Windows 쓰기 권한을 점검하여 갱신 작업이 즉시 진행될 수 있도록 준비한다.', 'UPIS, 표준시스템, 새올행정, 민원행정연계, GPKI, 서버인증서, 인증서갱신, gpkisecureweb, 보안모듈, 쓰기권한, PBK', 14, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. UPIS 표준시스템(톰캣)을 재기동한 후 민원행정 연계 서비스 정상 동작을 AP 서버별로 각각 확인한다.', '이중화 AP 서버 환경에서는 PBK 갱신·포트 허용 작업 후 반드시 AP1·AP2 서버 양쪽에 동일한 설정이 적용되었는지 점검하고, 인증서 갱신 주기에 맞춰 사전 PBK 재생성 일정을 수립한다.', 'UPIS, 표준시스템, 민원행정 연계, PBK, GPKI, 새올, 공개키, 인증서, connect refused, 포트 차단, 톰캣, AP서버, LDAP, GPKISecureWEB, 900오류, 1507오류', 13, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 작성 완료된 문서를 해당 기관 담당 주무관에게 이메일로 첨부하여 송부한다.', '공공데이터 보유현황 조사 주기(연간 등)에 맞춰 스크립트 및 문서 작성 절차를 표준화하고 기관별 일정을 사전 공지하여 지원 요청을 분산시킨다.', 'UPIS, 공공데이터 보유현황, 표준시스템, 데이터베이스 정의서, 컬럼 정의서, 스크립트 실행, AP서버, DB서버, 문서 작성, 기술지원', 12, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
8. 표준시스템에서 새올행정 연계 및 전체 정상 동작 최종 확인', 'GWS 설치 시 레지스트리 JvmMx 값을 6144 이상으로 사전 설정하고, GPKI PBK 공개키 인증서 만료일을 주기적으로 모니터링하여 만료 전 갱신 절차를 수행할 것.', 'UPIS, 표준시스템, GWS, 용도지역, 지도미표출, JvmMx, 레지스트리, GPKI, PBK, 공개키갱신, 새올민원, 기간만료, 웹서버재기동', 11, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 재발 방지를 위해 Tomcat 서비스 시작 유형을 ''''수동''''에서 ''''자동''''으로 변경', 'Windows 서비스 관리자에서 Tomcat 7.0 서비스의 시작 유형을 ''''자동''''으로 설정하여 서버 재기동 시 자동으로 구동되도록 구성한다.', 'UPIS, 표준시스템, Tomcat, Tomcat7.0, 서비스 미구동, 자동시작, Windows 서비스, 프로세스 강제종료, 도시계획정보체계, 서버 재기동', 10, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. 무한루프 등 원격 조치 불가 현상 발생 시 현장 방문 지원 요청 및 방문 전 데이터 정시성 검수 완료 확인', '재해취약성 데이터 생성 단계에서 도형 필드 타입을 전부 string으로 통일하고, SIGNGU_SE 등 필수 코드 컬럼 누락 여부를 업로드 전에 사전 검수하는 절차를 표준화한다.', 'UPIS, 표준시스템, 재해취약성, 데이터 업로드 오류, 도형 데이터, 속성 데이터, TN_EVAL_DRUT, TN_EVAL_HEAT, TN_EVAL_RAIN, TN_EVAL_SNOW, TN_EVAL_SSEA, TN_EVAL_WIND, SMAR_CD, SAMR_NM, SIGNGU_SE, 데이터 타입, string, double, 무한루프, 시군구코드', 10, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. 정상 확인된 속성 MDB 파일을 담당자(관리자)에게 전달하고 완료 처리한다.', 'UPIS 서버 설치·업그레이드 시 서버 OS 로케일과 오라클 NLS_CHARACTERSET 설정이 한글(ko_KR / AL32UTF8)로 일치하는지 사전에 확인하고, 초기 속성 MDB 파일을 별도 보관해 둔다.', 'UPIS, 표준시스템, 한글 깨짐, 로케일, 오라클, NLS_CHARACTERSET, ko_KR, 속성 데이터, 도형 데이터, MDB, 데이터 백업', 8, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 테스트 결과를 확인하고 기관 담당자에게 연계 동작 상태를 안내한다.', 'UPIS 표준시스템 신규 구축 또는 업데이트 후에는 LURIS 연계 테스트를 사전에 수행하여 연계 정상 여부를 운영 전 확인한다.', 'UPIS, LURIS, 토지이용규제정보서비스, 표준시스템, 연계 테스트, 도시계획정보체계', 7, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. 표준시스템 새올행정 연계 정상 작동 여부 최종 확인', 'GPKI 공개키 인증서의 만료일을 주기적으로 점검하고, 만료 전 PBK 재생성 및 배포 절차를 사전에 수행하여 연계 중단을 예방한다.', 'UPIS, 표준시스템, 민원행정, 새올행정, GPKI, PBK, 공개키인증서, 인증서만료, 1301오류, GPKIPASS, TC_SAEALL_INFO, GSS', 7, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. GWS 재기동
6. UPIS 8.0 재기동
7. 표준시스템에서 지도 조회 정상 여부 확인 후 담당자에게 완료 통보', 'GSS.lax 의 JVM 메모리 설정을 min 512MB / max 1024MB 이상으로 유지하고, 정기적으로 GSS 로그를 점검하여 메모리 경고 징후를 사전에 감지한다.', 'UPIS, GSS, GWS, 표준시스템, 지도 오류, 메모리 부족, GSS.lax, JVM 메모리, 용도지역, 필지정보조회, GSS 연결 실패', 6, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 재배포 후 해당 레이어가 정상적으로 표시되는지 검증한다.', 'UPIS.xml 레이어 연결 명칭은 언더바 등 특수문자 오기재 여부를 포함하여 배포 전 레이어명 일치 여부를 검수한다.', 'UPIS, UPIS.xml, 표준시스템, 레이어 연결 오류, 도시계획이력도, 도시계획계획도, UPIS_P_RBEP, 도시계획집행시설, 도시계획시설집행시설, XML 수정, 재배포', 6, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 조치 후 해당 조회 화면에서 정상 표출 여부 재확인', 'UPIS 표준시스템 구축·업그레이드 시 필수 테이블(TN_DSTPLAN_ATCHFILE 등) 목록 체크리스트로 누락 여부를 사전 검증하고, 코드 참조 테이블의 코드값이 쿼리 기준값과 일치하는지 데이터 정합성 검사를 정기적으로 수행한다.', 'UPIS, 표준시스템, 용도지역 미표출, 고시정보 미표출, 무한루프, TN_SPFC_WTNNC, PMC0002, PMC0003, TN_DRW_IMAGE, TN_DRW_IMAGE_MAPNG, RECORD_CODE 중복, TN_DSTPLAN_ATCHFILE, 테이블 누락, 데이터 오류, 도시계획정보체계', 6, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 집행도 범례 미표출 등 추가 이상 증상은 사업단(담당 주임)에게 처리 요청 및 결과 확인', 'UPIS 패치 배포 시 upis.properties의 새주소 API 공개키 버전을 배포 체크리스트에 포함하여 구버전 키가 잔존하지 않도록 사전 점검한다.', 'UPIS, 표준시스템, 자연어검색, 새주소 API, 공개키, upis.properties, Tomcat, 집행도 범례, 조회 오류, 패치 오류', 6, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 아산시 등 기관별 재해취약성 상세 필지 쿼리가 표준 호출 방식과 다를 경우 사업단에 문의하여 쿼리 정합성을 확인한다.', '재해취약성 데이터 업로드 전 UPIS 표준시스템 패치 버전을 사전 점검하고, 신규 패치 배포 시 즉시 적용한다.', 'UPIS, 표준시스템, 재해취약성, 업로드 오류, GSS, GWS, Tomcat, 톰캣 재기동, 패치 오류, 시스템 중지, 도시계획정보체계', 5, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 설정 파일 수정 후 표준시스템을 재기동하고 용도지역 검색·필지 클릭 정상 동작 여부를 확인한다', '패치 배포 시 필수 디렉토리 목록과 upis.properties 포트 항목(WMS·WFS 모두)을 체크리스트로 관리하여 적용 누락 여부를 사전 검증한다.', 'UPIS, 표준시스템, 패치오류, 구동오류, UpdaterUpis.bat, upis.properties, WMS, WFS, 포트누락, 디렉토리누락, class파일누락, 뷰데이터, 메타뷰, 도시계획정보체계', 5, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. GWS·GSS를 순서대로 재기동하고 표준시스템 정상 작동을 확인·보고한다.', 'Oracle Archive 로그 디스크 사용률을 주기적으로 모니터링하고, UPIS 계정 비밀번호 정책(만료·잠금) 변경 시 수직자료연계 모듈 설정과 동기화하는 절차를 수립한다.', 'UPIS, 표준시스템, Oracle, account unlock, GSS, GWS, 리스너, Archive 로그, 용량 부족, DB 접속 오류, 계정 잠금', 4, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 최종 확인 완료 후 업무 요청자 및 관련 담당자에게 처리 결과를 보고한다.', '구역계 파일 납품 시 데이터 형식(면형식 필수)과 좌표계 설정값을 사전 검수 체크리스트에 포함하여 적용 전 오류를 차단한다.', '토지적성평가, 구역계 파일, 좌표계 오류, 선형식, 면형식, Desktop, 토적 프로그램, 데이터 검수', 4, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. 결과 공유: 처리 결과를 담당자(엔진사 또는 내부 담당)에게 메일 등으로 통보한다.', '수직적 자료연계 전 기초 지자체별 공간 데이터의 좌표계(투영·원점) 일치 여부를 사전 검수 단계에서 일괄 확인한다.', 'UPIS, 표준시스템, 지도 오류, 레이어 어긋남, 좌표계, 원점 좌표계, 수직적 자료연계, 용도지역 미표출, ArcGIS Desktop, QGIS, 데이터 구축 오류, 충청북도, 광명시, 산청군', 4, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. (관리자 계정 삭제 사례) sysadmin1 관리자 계정을 신규 생성하고 담당 주무관 권한에 표준시스템 관리자 전체 권한을 부여하여 복구한다.', '정기적으로 관리자 계정(sysadmin1 등) 존재 여부 및 계정 잠금 상태를 점검하고, 계정 삭제 전 반드시 백업 관리자 계정을 확보해 둔다.', 'UPIS, 표준시스템, 로그인 오류, 계정 잠금, TN_USER_NEW, 비밀번호 초기화, 사용자 계정, sysadmin1, 데이터 오류, 도시계획정보체계', 4, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
7. 정상 동작 여부 테스트 진행
8. 처리 완료 후 업무 요청자에게 결과 보고', '표준시스템 기동 후 로그를 주기적으로 점검하고, 데이터 업로드 항목(테이블 정보, 고시도면 등)은 도입 초기 체크리스트를 통해 누락 없이 일괄 처리한다.', 'UPIS, 표준시스템, 구동 오류, 데이터 업로드, TN_DSTPLAN_DEPT, TN_SERVER_INFO, 고시도면, 입안자료, 재기동, 도시계획정보체계', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 용량 확보 후 UPIS 표준시스템 서비스를 재기동하여 정상 구동 여부를 확인한다.', '서버 C드라이브 잔여 용량을 주기적으로 모니터링하고, TEMP 및 uwes/store 폴더에 누적 파일이 쌓이지 않도록 정기 정리 정책을 수립한다.', 'UPIS, 표준시스템, GWS, uwes/store, 디스크 용량 부족, C드라이브, TEMP 폴더, 서버 장애, 시스템 기동 오류', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 배포 후 지도 레이어 정상 표시 여부 확인', 'UPIS.xml 변경 시 레이어명 정확성을 사전 검증하고, 배포 전 테스트 환경에서 레이어 연결 상태를 확인하는 절차를 수립한다.', 'UPIS.xml, 표준시스템, UPIS, 도시계획정보체계, 레이어 오류, 도시계획이력도, UPIS_P_RBEP, 도시계획시설집행시설, XML 패치, 지도 오류, 레이어명, 재배포', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
8. AP 서버가 종료된 지자체(의령군 등)는 서버 기동 후 Oracle 연결 상태 재확인', '수직자료 연계 운영 전 하위 지자체 전체의 포트(80·7070·8080) 개방 여부와 AP 서버 기동 상태를 주기적으로 점검하고, AP1·AP2 서버 간 설정 파일을 동기화 상태로 유지하여 단일 서버 장애 시 빠른 전환이 가능하도록 관리.', 'UPIS, 수직자료연계, PBK, 새올, 인증서, AP서버, Oracle, 포트연계, 민원행정, 경상남도, 대구광역시', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 교체 후 민원행정 연계 기능을 재테스트하여 정상 동작을 확인한다.', 'GPKI 서버인증서 만료일을 주기적으로 점검하고, 만료 30일 전에 갱신 절차를 진행하여 연계 단절을 사전에 방지한다.', 'UPIS, 표준시스템, 민원행정연계, GPKI, 서버인증서, 인증서갱신, SSL, 인증서만료, 연계장애', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 섬네일 미표출 문제의 경우 섬네일 폴더에서 해당 파일 존재 여부를 확인하고, 파일이 없으면 섬네일을 재생성하여 지정 경로에 저장', '정기적으로 연속지적도 및 행정경계 테이블의 코드 정합성을 점검하고, 공간데이터 백업을 최신 상태로 유지하여 유실 시 신속히 복구할 수 있도록 관리함.', 'UPIS, 표준시스템, emd_cd, lp_aa_emd, LA_PA_EMD, LP_PA_CBND, 연속지적도, 행정경계코드, 데이터유실, 섬네일, 필지검색, 조회오류, 백업재탑재', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 표준시스템 재구동 후 정상 표출 여부 테스트', 'AP 서버 전원 상태를 주기적으로 모니터링하고, 비정상 종료 시 담당자에게 즉시 알림이 가도록 서버 감시 체계를 구성함.', 'UPIS, 표준시스템, AP서버, AP_Server, Spatial Server, 서버 전원 OFF, 서버 재기동, 장애복구', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 재발급 완료 후 새올행정 연계 정상 동작 여부를 확인한다.', 'GPKI 서버인증서 만료일을 주기적으로 점검하여 만료 전에 사전 재발급을 진행한다.', 'UPIS, 표준시스템, 새올행정, GPKI, 서버인증서, 인증서만료, 민원행정연계, 연계오류', 3, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 라이선스 갱신 필요 사실을 담당 부장(정영훈) 및 담당 대리(담당자)에게 전달하여 갱신 조치를 요청한다.', '임시 라이선스 사용 시 만료일을 사전에 등록하고, 만료 전 갱신 절차를 미리 진행한다.', 'UPIS, 표준시스템, 민원행정연계, 라이선스 오류, 임시라이선스, 라이선스 갱신, 엔진', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 지도 표출 여부를 재확인하여 정상 출력됨을 확인한다.', '공간 데이터(용도지역 등) DB 업데이트 후 반드시 좌표계(공간정의) 설정 여부를 검수 항목에 포함하여 누락을 사전에 방지한다.', '표준시스템, UPIS, 지도 미표출, 좌표계, 공간정의, 용도지역, LOD, 연혁도, DB 업데이트, 레이어', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-05-06-01', '지원', 'UPIS', '토지적성평가', '토지적성평가 라이센스 발급', '미상(구양식)', '토지적성평가 내부용 라이센스를 1년 단위로 갱신 발급하여 사용 가능 상태로 복원하였다.', '토지적성평가 시스템의 라이센스가 만료되거나 신규 발급이 필요한 상황이 발생하였다. 해당 기능의 정상 사용을 위해 라이센스 발급 처리가 요청되었다.', '원인 불명확(기록상 미상) — 라이센스 만료 주기(1년) 도래에 따른 갱신 필요로 추정된다.', '1. 토지적성평가 라이센스 발급 요청 접수(내부용 여부 확인)
2. 1년 단위 갱신 기준으로 라이센스 생성
3. 발급된 라이센스를 해당 기관(경산 본사)에 전달 및 적용 완료 확인', '라이센스 만료 30일 전 사전 알림 체계를 마련하여 갱신 지연으로 인한 서비스 중단을 예방한다.', '토지적성평가, 라이센스, 라이센스 발급, 라이센스 갱신, 내부용 라이센스, 1년 단위 갱신', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 표준시스템에서 민원행정 연계 상태를 재확인하여 정상 연계 여부를 검증한다.', '부서 조직 개편 또는 GPKI 인증서 갱신 시 TC_SAEALL_INFO 테이블 설정 값을 즉시 동기화하는 절차를 운영 매뉴얼에 포함한다.', 'UPIS, 표준시스템, 민원행정연계, TC_SAEALL_INFO, 부서코드, GPKI, 인증서패스워드, 연계오류', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. UPIS 표준시스템 접속 후 지도 표출 및 주요 기능 정상 동작 확인', 'Apache Tomcat 8.0 UPIS 서비스에 자동 재시작 정책(윈도우 서비스 복구 옵션 또는 모니터링 스크립트)을 적용하여 엔진 다운 시 신속히 복구되도록 설정한다.', 'UPIS, 표준시스템, Apache Tomcat, Tomcat 8.0, 지도 미표출, 엔진 다운, 페이지 장애, 남양주시, 제주특별자치도, 도시계획정보체계', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. 정보연계신청서를 대리 작성한다.
5. 작성 완료된 신청서를 기관 또는 관련 기관에 송부한다.', '기관 담당자에게 정보연계신청서 작성 절차와 양식을 사전 안내하여 자체 작성 역량을 확보하도록 한다.', 'UPIS, 표준시스템, 새올, 정보연계신청서, 연계신청, 대리작성, 제주특별자치도, 지오투', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-04-05-01', '장애', 'UPIS', '토지적성평가', '토지적성평가 로그인 오류', '인터넷 접속 오류', '토지적성평가 로그인 실패 장애는 인터넷 연결 불량이 원인이었으며, 네트워크 연결 복구 후 정상 로그인이 확인되었다.', '토지적성평가 프로그램 실행은 정상이나 로그인 시도 시 오류가 발생하여 진입이 불가능하다.', '사용자 단의 인터넷 연결 불량으로 인해 인증 서버와의 통신이 차단되어 로그인 처리가 실패하였다.', '1. 사용자에게 토지적성평가 프로그램 실행 여부와 로그인 단계에서의 오류 발생 시점을 확인한다.
2. 사용자 PC의 인터넷 연결 상태(외부망 접속 가능 여부)를 직접 확인하도록 요청한다.
3. 인터넷 연결 이상 발생 시 네트워크 복구(케이블 재연결, 공유기 재기동 등)를 수행한다.
4. 인터넷 연결 복구 후 토지적성평가에 재로그인하여 정상 진입을 확인한다.', '토지적성평가 로그인 오류 발생 시 프로그램 오류 전에 사용자 단 인터넷 연결 상태를 먼저 점검하는 절차를 안내 매뉴얼에 포함한다.', '토지적성평가, 로그인 오류, 인터넷 연결 불량, 네트워크 장애, UPIS, LSA', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 위 조치 후에도 증상이 지속될 경우 담당 주무관 PC의 개별 장애 여부를 확인하고 현장 지원 또는 원격 지원으로 연결한다.', 'Tomcat 메모리 설정을 운영 초기에 충분히 확보(권장 4096MB 이상)하고, 브라우저 팝업 차단 해제 설정을 사용자 매뉴얼에 필수 항목으로 안내하여 반복 문의를 예방한다.', 'UPIS, 표준시스템, 고시정보, 조서정보, 필지정보조회, KRAS, Tomcat, 팝업차단, 캐시삭제, 메모리증설, 조회오류', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 표준시스템에서 고시문·고시도면이 정상 표출되는지 확인한다.', 'Tomcat server.xml의 docBase 경로는 운영체제의 실제 디렉터리명(대소문자)과 정확히 일치하도록 초기 설치 시 검수 항목에 포함한다.', 'UPIS, 표준시스템, Tomcat, server.xml, docBase, 대소문자, 고시문, 고시도면, Context, 조회오류', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
8. 웹서버 및 톰캣 재기동 후 수직적자료교환 정상 동작 확인', 'uwes.war 패치 전 WEB-INF/upis.properties 및 web.xml을 별도로 보관하고, 패치 절차 체크리스트에 설정 파일 복원 단계를 필수 항목으로 포함.', 'UPIS, 표준시스템, uwes, uwes.war, 수직적자료교환, GWS, GeoNURIS WebServer, 톰캣, upis.properties, web.xml, 패치, deploy', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. SSO 로그인 재시도 및 정상 연계 여부 확인', '신규 사용자 등록 또는 담당자 변경 시 TN_USER_NEW 테이블에 새올 아이디를 즉시 갱신하는 절차를 운영 매뉴얼에 포함시킨다.', 'SSO, 표준시스템, UPIS, TN_USER_NEW, 새올, SSO 연계 오류, 인증 오류, 사용자 테이블', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
5. Tomcat 서비스 등록이 불가한 경우, 서비스 등록 방식 대신 모듈 활성화(모듈활성) 방식으로 엔진을 기동한다.', '표준시스템 버전 패치 전 테스트 서버에서 맵 레이어 세팅 및 Tomcat 기동 방식을 사전 검증한 후 운영 서버에 적용한다.', 'UPIS, 표준시스템, 지도 오류, 맵 레이어, 레이어 겹침, Tomcat, 서비스 등록, 모듈활성화, 버전 패치, 도시계획정보체계', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 외부 배경지도 API(다음 지도) 연동 상태를 확인하여 배경지도 미수신 여부 파악 후 담당자에게 자체 확인 및 처리 요청', '지적도 캐시 레벨 범위를 운영 환경에 맞게 제한하고, 14레벨 초과 확대 기능 사용 시 스케일 변수 및 파일 수 증가에 따른 부하를 사전에 검토하여 캐시 정책을 수립한다.', 'UPIS, GSS, GWS, 지적도 캐시, nnp_gis_base, 캐시 레벨, 지도 미표출, 다음 지도 API, 배경지도, /gisdata/cache, 엔진', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
...
4. GWS 서비스 재기동
5. 표준시스템 서비스 재기동 후 지도 표출 정상 여부 확인', 'GWS store 백업파일 보관 정책을 수립하고 주기적으로 오래된 백업파일을 정리하여 C드라이브 여유 공간을 일정 수준 이상 유지한다.', 'UPIS, 표준시스템, GWS, 지도 오류, 디스크 용량 부족, store 백업파일, C드라이브 풀, 서비스 재기동', 2, TRUE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-01-04-01', '장애', 'UPIS', '기타', '토지적성평가 라이센스 발급', '미상(구양식)', NULL, NULL, NULL, '* 토드(TOAD)에 속성데이터 Import 하였을 시에 한글데이터 깨짐 현상 - 김준혁 과장 해당 본인 PC에서만 한글이 깨져보이며, 실데이터는 깨지지 않음.', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-03-03-01', '장애', 'UPIS', '연계', '민원행정 연계', '미상(구양식)', NULL, NULL, NULL, '* 당진시 UPIS 표준시스템 민원행정 연계 1. 당진시 교체된 민원행정공통기반2단계 인증서값 수령 2. 당진시의 금번 교체된 민원행정공통기반2단계 인증서가 민원행정사업단에 등록되어있는지 확인 3. 당진시 UPIS서버에서 공개키값 발급 확인(PBK값) 4. 타지자체에서 당진시 공개키값 발급 확인 -', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-03-04-01', '장애', 'UPIS', '연계', '새올행정 연계 오류', '미상(구양식)', NULL, NULL, NULL, '1. GPKI error code = 1507 오류가 발생하여 새올민원 연계가 되지않음 2. 에러코드 인증서 확인 요청 3. 해당 서버 새올행정 인증서가 만료되어 PBK 모듈 사용하여 재발급 4. 새올행정 연계확인 및 메시지 보고 완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-01-02-01', '장애', 'UPIS', '기타', '시스템상에서 한글이 깨지는 현상', '미상(구양식)', NULL, NULL, NULL, '◯ 문제점 상세 법무부 테스트 시스템의 지도에서 한글이 깨지는 현상을 발견하고 연락옴 ◯ 원인분석 1. DB에서 한글의 깨짐 유무를 확인(정상표출) 2. Desktop의 속성테이블 조회기능을 이용하여 공간데이터 서버에서 불러오는 데이터의 한글의 깨짐 유무를 확인(정상표출) 3. WebServer에서 지도를 표출하여 한글이 깨짐 여부를 확인(비정상표출) ◯ 분석결과 WebServer상에서 한글이 깨지는 것을 확인하였으며 이는 지도에 설정된 라벨에 문제가 있는 것 임(보통은 OS폰트가 설정되지 않아서 발생하는 문제임) ◯ 조치사항 Linux한글 폰트를 다시 설정하고 해결이 되지 않으면 연락을 달라고 하였음', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-01-01-01', '장애', 'UPIS', '기타', '레이어 오류', '미상(구양식)', NULL, NULL, NULL, '완전 빈도형 작업 및 레이어 오류 발견 - 18.01.11 (목) 오전에 완전 빈도형 작업을 옥천군청에서 하던 도중 도시계획시설집행시설 (P_UQ151_1 ~ P_UQ159_1), 개발행위허가 미분류, 기타(H_UQ174_1, H_UQ174_2) 발견 - 개발행위허가는 레이어는 잘못된 레이어로 판명 (스타일이 구분되어있어야 하는데 레이어가 구분되어있으므로) - 도시계획이력도와 계획도안의 도시계획시설집행시설은 거의 모든 지자체가 _형식으로 위와 같이 되어있는 것으로 확인 - 18.01.11 (목) 오후에 잘못된 레이어명 수정 완료 - 현재 Xml 오류 계속해서 찾는 중', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-01-03-01', '장애', 'UPIS', '기타', '울릉도의 주소가 표시되지 않는 문제', '미상(구양식)', NULL, NULL, NULL, '◯ 조치사항 지도가 맞지 않는 것을 법무부 담당에게 알려주었으며 담당은 유볼트 장현길이사님을 통해 틀어진 지도를 맞춘다고 함(정도에서는 더 이상 관여할 일이 없음)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-03-02-01', '장애', 'UPIS', '연계', 'UPIS표준시스템 오류', '미상(구양식)', NULL, NULL, NULL, '1. UPIS8.0 톰캣 구동상태에서 재기동을 한 에러가 발생하여 정상종료 후 재기동 2. GWS서버도 종료시켜두었다고 하여 재기동 3. AP1, AP2번 서버 확인 정상 확인 1. AP1서버 톰캣이 서비스에서 실행되지 않음 2. 이벤트 로그를 확인하라는 에러메시지가 뜨면서 서비스 실행 중지 3. UPIS8w 샘플 파일을 붙여넣었으나 동일 오류발생 4. 톰캣을 새로 설치 후 정상작동 완료 5. AP2번 서버는 GWS 기동이 되어 있지 않아 오류가 발생, 재기동하여 문제 해결', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-02-01', '장애', 'UPIS', '표준시스템', 'UPIS 표준시스템 및 엔진 분기 점검', '미상(구양식)', NULL, NULL, NULL, '1. GSS 꺼져있음 (구동화면에서 타이핑이 되어있어 GSS 중지되어 재기동) 2. 웹서버 접속하여 비어있는 레이어 추가', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-01-01', '장애', 'UPIS', '표준시스템', 'GIS 엔진 정기점검', '미상(구양식)', NULL, NULL, NULL, '1. GIS 엔진 점검 실시 - 재해취약성 구축되어 있음', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-04-04-01', '장애', 'UPIS', '토지적성평가', '토지적성평가 기타', '기타', NULL, NULL, NULL, '· 구역계 파일을 검토 등록하면 Error가 발생

가. Error 문구를 확인
나. 확인결과 토적시스템은 파일명이 19자를 초과하면 등록 할 수 없도록 처리되어있음', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-04-03-02', '장애', 'UPIS', '토지적성평가', '토지적성평가 기능 오류', '실행 권한 부여', NULL, NULL, NULL, '1. 일부 기능에 오류가 발견됨
2. window 버전이 업데이트 됨에 따라 관리자 권한으로 변경하여 실행하여 완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-04-02-01', '장애', 'UPIS', '토지적성평가', '토지적성평가 구동 오류', '메모리 오류', NULL, NULL, NULL, '· 토지적성평가 미 실행으로 인한 문의

가. 인터넷 연결확인
나. 라이선스 확인
다. 메모리 설정 확인
라. 확인결과 메모리 설정이 잘못되어 있는것을 확인하고 수정
마. 테스트를 진행', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-04-03-01', '장애', 'UPIS', '토지적성평가', '토지적성평가 기능 오류', '데이터 처리 방식', NULL, NULL, NULL, '· 지도상에서 확인했을 때 평가대상지역 전체가 별도보전지역으로 판단되었으나 입안검토가 진행되는 현상이 발생함
...
다. 업무요청자에게 상황을 설명 드리고 상세 보고서를 작성함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-04-01-01', '장애', 'UPIS', '토지적성평가', '구역계 파일을 검토등록하면 Error 발생', '미상(구양식)', NULL, NULL, NULL, '구역계 파일명 수정 화면', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-03-05-01', '장애', 'UPIS', '연계', '울주군 새올민원 연계에러', '미상(구양식)', NULL, NULL, NULL, '1. UPIS 표준시스템에서 울주군 GPKI 900, 1507오류 발생 2. 인증서 확인 & 갱신유무 확인 3. SVR3730068001PBK.cer 갱신하여 GPKI, GPKISecureWEB 붙여넣기 4. UPIS 시스템 재기동 5. AP1 서버 확인 및 AP2서버에 동일하게 적용하여 완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 표출 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
2. 처리 :
· 1) 변경된 부서코드 수령받아 TC_SAEALL_INFO 테이블 코드값 변경하여 해결', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 해당 주무관의 유저의 시군구 코드를 변경하여 문제해결', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
1. 민원행정연계 오류
2. GPKI PBK값 도출 및 민원행정 표출 완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
6. 표준시스템 새올행정 확인
7. 정상작동 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
· 아이디가 administrator -> ypupisap로 변경된겻을 확인함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
나. 공개키 인증서를 다시 갱신함
다. 표준시스템 재구동
라. 테스트 진행', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 표준시스템 정상작동 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-03-09-01', '지원', 'UPIS', '엔진', '전 지자체 엔진버전 조사', '미상(구양식)', NULL, NULL, NULL, '01. 02 ~ 18.01.05) - 전 지자체 Desktop CCI회사로 설치 되어있는 곳 확인 - GWS 설치파일 버전 확인 및 설치경로 조사 - GSS 설치 최신화 조사 설치파일 전수조사.xls 파일에 정리하여 기록 후 팀원 전체에게 메일로 발송', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-03-08-01', '지원', 'UPIS', '엔진', '완전 빈도형 작업', '미상(구양식)', NULL, NULL, NULL, '01. 02 ~ 18.01.05) - 완전 빈도형 모든 레이어 수집', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-02-02-01', '지원', 'UPIS', '기타', '도로명사업단 과업 요청 답변서 작성', '미상(구양식)', NULL, NULL, NULL, '답변서(ver1)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-02-03-01', '지원', 'UPIS', '기타', '영월군 착수계', '미상(구양식)', NULL, NULL, NULL, '* 영월군 착수계 투입 인력 검토 및 서명.', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-01-02-01', '지원', 'UPIS', '공원녹지', '공원녹지 데이터', '데이터 백업', NULL, NULL, NULL, '· 평택 공원녹지 데이터를 다운로드 받아 달라고함

가. 평택 UPIS 원격에 접속
나. DB Tool을 이용하여 도형을 내려받음
· 공원녹지시스템과 표준시스템은 같은 DB서버를 사용하고 있음
다. 업무요청자에게 메일을 통해 데이터를 전달함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-02-01-01', '지원', 'UPIS', '기타', 'DB 업데이트', '미상(구양식)', NULL, NULL, NULL, '1. Oracle Client 설치 2. DB 업데이트', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-01-01-01', '지원', 'UPIS', '공원녹지', '공원녹지', '공원녹지 데이터', NULL, NULL, NULL, '데이터 백업', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
해결예정
6. 18.02.01에 JEUS 엔지니어와 함께 방문하여 설정에 문제가 되는 부분(JEUS 옵션설정에 문제가 있었음)을 확인하였으며 수정을 통해 문제를 해결하였음(담당자 방문)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
5. EBMS모듈 삭제 후 재설치
6. 정상적으로 작동완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-03-01', '장애', 'UPIS', '표준시스템', 'UPIS 표준시스템 이력 미표출', '미상(구양식)', NULL, NULL, NULL, '* 울산광역시 해당 필지 이력 미표출하는 현상 - 확인 결과 특정 필지뿐만 아니라 모든 필지의 이력이 미표출되고있었으며, GSS / GWS 엔진이 정상적으로 실행되지 않고있었음. - 엔진 재기동 후 확인하여 해결함.', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-05-01', '장애', 'UPIS', '표준시스템', 'UPIS.xml 복구 요청', '미상(구양식)', NULL, NULL, NULL, '1. LOD 실행 중에 UPIS.xml 내용이 삭제됨 (다른 지자체 xml 파일 복사하여 내용 수정 후 복구)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 담당 주무관 자리에서 정상작동 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-11-01', '장애', 'UPIS', '표준시스템', '진주시 UPIS 8.0 시스템 안켜짐', '미상(구양식)', NULL, NULL, NULL, '12. 29 11:00 - 17.12.29 11:30 시스템 이상으로 서비스에서 GWS, UPIS 8.0 내렸다고 하는데 들어가 보니 이상없고 재기동 및 중지 정상작동 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-12-01', '장애', 'UPIS', '표준시스템', '표준시스템 LURIS 연계', 'LURIS 정식 연계', NULL, NULL, NULL, '패치진행(LURIS 테스트)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('1-05-10-01', '장애', 'UPIS', '표준시스템', '지구단위 계획 미표출 데스크탑 확인', '미상(구양식)', NULL, NULL, NULL, '1. UPIS 시스템에서 지구단위계획 구역 미표출 확인 2. 데스크탑 라이센스가 다르다는 에러가 확인 3. Mac Address 확인하였는데 변경됨을 확인 4. 데스크탑 재발급 및 적용하여 문제 해결 5. UPIS 시스템 상에서 신규XML 업로드하여 XML 변경 6. 서버 재기동 및 표준시스템 표출 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-04-01', '지원', 'UPIS', '표준시스템', 'UPIS 표준시스템 및 GIS 엔진 점검', '미상(구양식)', NULL, NULL, NULL, '1. 표고도, 경사도 미표출(해결)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-03-01', '지원', 'UPIS', '표준시스템', 'UPIS 표준시스템 Vworld 미표출', '미상(구양식)', NULL, NULL, NULL, '1. Vworld 미표출 (xml 수정하여 해결) 2. 유지보수 계약관계가 되어있지 않아 uwes 패치 진행', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-01-01', '지원', 'UPIS', '표준시스템', 'DB 업데이트', '미상(구양식)', NULL, NULL, NULL, '1. 집행도, 현황도, 이력도(나주, 담양제외) DB 업데이트 2. 신안군 속성데이터, 영암군 업데이트 3. LOD 실행', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-05-07-01', '지원', 'UPIS', '토지적성평가', '토지적성평가 사용자 교육', '미상(구양식)', NULL, NULL, NULL, '1. 토지적성평가 사용자 교육 실시', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-05-03-01', '지원', 'UPIS', '토지적성평가', '토적 납품용 제품 제작', '미상(구양식)', NULL, NULL, NULL, '* 담양 토지적성평가 납품용 CD 및 케이스 제작', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-04-01-01', '지원', 'UPIS', '연계', '전자결재 및 GPKI 세팅', '미상(구양식)', NULL, NULL, NULL, '1. 전자결재 세팅 완료 - 지자체 전자결재 서버 모듈 재기동 해야함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-05-01-01', '지원', 'UPIS', '토지적성평가', '구역계 Mapping 테스트 진행', '미상(구양식)', NULL, NULL, NULL, '파일 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
· 기본 비번으로 변경함
다. 업무요청자 및 임석호 계장님에게 보고함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
2. sysadmin1 계정 접속하여 관리자 탭에서 사용자 삭제', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
다. 다시 실행함
라. Tomcat7.0이 다운되어 발생한 문제로 확인하고 업무요청자에게 보고함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-05-01', '지원', 'UPIS', '표준시스템', 'UPIS 표준시스템 및 엔진 점검', '미상(구양식)', NULL, NULL, NULL, '1. 정기점검 실시 (완료) 2. GSS로그 예전 데이터 삭제 3. 재해취약성 데이터 구축 되어있음', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-08-01', '지원', 'UPIS', '표준시스템', '연혁도 수정 요청', '미상(구양식)', NULL, NULL, NULL, '1. 데이터 업로드 - 연혁도 추가 (xml 수정) 2. LOD 실행', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-09-01', '지원', 'UPIS', '표준시스템', '작업시간 (09:30 ~ 10:10) 1. 계약 2.', '미상(구양식)', NULL, NULL, NULL, '1. 계약 2. 정기점검 실시 - store 정리 - log파일 17년 12월만 남겨놓고 삭제', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-11-01', '지원', 'UPIS', '표준시스템', '지도상에서 Layer가 어긋나는 문제', '미상(구양식)', NULL, NULL, NULL, '두 Layer 맵핑 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-12-01', '지원', 'UPIS', '표준시스템', '착수계 작성', '미상(구양식)', NULL, NULL, NULL, '대외공문신청서 상신', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-07-01', '지원', 'UPIS', '표준시스템', '사업관련 서류 준비 및 우편발송', '미상(구양식)', NULL, NULL, NULL, '신원진술서(약식)작성', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-05-04-01', '지원', 'UPIS', '토지적성평가', '토지적성평가 고흥군 Test', '미상(구양식)', NULL, NULL, NULL, '토지적성평가 고흥군 Test - 18.01.08 14:30분경에 토지적성평가 활용매뉴얼을 참고하여 Test 시작 - 18.01.08 17:00분경에 데이터 이상으로인하여 고흥군 데이터로 변경 - 18.01.09 08:10분경에 내 데스크탑에 토지적성평가 라이센스 발급 후 설치 - 18.01.09 14:00분경에 분석 마침, 노트북으로 옮기는 과정에서 데이터 손실 발생 - 18.01.10 14:00분경에 다시 노트북에서 분석 마친 후 컨버터 실행까지 완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-05-02-01', '지원', 'UPIS', '토지적성평가', '보전지역에서 입안검토가 진행되는 문제', '미상(구양식)', NULL, NULL, NULL, '토지적성평가 확인서 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-18-07', '지원', 'UPIS', '표준시스템', '표준시스템 문서 작성', '영월군 착수계', NULL, NULL, NULL, '1. 영월군 착수계 투입 인력 검토 및 서명', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
18. 10.30
1. 사업단 문제해결', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
2. 담당자에게 전달', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
2. 처리 :
· 1) 스크립트 실행
· 2) 공공데이터 보유현황 문서 작성(데이터베이스 정의서, 컬럼 정의서 등)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
나. 완료후 업무요청자에게 내용을 보고함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
2. 담당자에게 해결요청 함 (문제가 있음)', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
나. 문서작성 완료후 업무요청자에게 메일로 발송', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
INSERT INTO tb_ops_kb (kb_code, gubun, sys_type, category, symptom, cause, summary, symptom_desc, cause_desc, action, prevention, keywords, case_count, rewritten) VALUES ('2-06-18-01', '지원', 'UPIS', '표준시스템', '표준시스템 문서 작성', 'LURIS 연계 키 확인', NULL, NULL, NULL, '문서 작성', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
1. 확인 : 포트허용 문서 작성
2. 처리 :
· 1) 기존 UPIS 도입시 문서를 바탕으로 포트허용 문서 작성하여 전달', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
1. 확인 : 라이선스 발급 내용 확인
2. 처리 :
· 1) 데이터베이스 정의서 엔진 및 서버 부문 작성하여 리턴', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
1. 확인 : 사업단 문의 유도
2. 처리 :
· 1) 수직모듈 미구동 지자체 사업단에 문의토록 주무관님에게 유도', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 합천군은 공문요청에 UPIS IP, KRAS IP가 잘못기입되어 있어 박정수 차장에게 수정 후 요청', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
· 2) 자료에 대한 피드백 정리 및 수정', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
1. 확인 : uwes 패치
2. 처리 :
· 1) uwes 패치 완료', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
5. 현황 파악 완료 및 문서 작성하여 최승환 주무관에게 메일 송부', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 해당값을 입력 하여 문제해결
4. 표준시스템에서 고시정보 지도 하이라이팅 기능 확인결과 정상표출 확인', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
나. 고시문 출력은 문제가 없음
다. 확인후 업무요청자에게 보고함', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
...
3. 표준시스템 기능 전반적 검토 후 해결', NULL, NULL, 1, FALSE) ON CONFLICT (kb_code) DO NOTHING;
