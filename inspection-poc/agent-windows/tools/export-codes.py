...
사용 (DB 좌표는 환경변수로 주입 — 실 좌표 하드코딩 금지, 운영DB 직결):
    set DB_HOST=...      set DB_PORT=...      set DB_NAME=...
    set DB_USER=...      set DB_PASSWORD=...
    python tools/export-codes.py
미설정 시 더미(localhost:5432/swdept_local)로 폴백 → 운영 접속 안 됨(의도적).
의존: psycopg2  (pip install psycopg2-binary)
"""
import os, sys, json, datetime
import psycopg2

# 실 DB 좌표는 repo 에 두지 않는다(시크릿/운영 안전). 환경변수 미설정 시 더미 placeholder.
DB = dict(
    host=os.environ.get("DB_HOST", "localhost"),
    port=int(os.environ.get("DB_PORT", "5432")),
    dbname=os.environ.get("DB_NAME", "swdept_local"),
