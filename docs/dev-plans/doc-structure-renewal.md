...
- `docs/plans/` 33개 / `docs/dev-plans/` 37개 / `docs/audit/` 16개 / docs 총 96 / repo 총 106 / root .md 5
...
**링크 검증 스크립트 파일 2개 작성**:
- `docs/dev-plans/docs-renewal-tools/scan-inventory.sh` — Before/After 인벤토리 생성
- `docs/dev-plans/docs-renewal-tools/verify-links.py` — Markdown/HTML 링크 + 파일 존재 + 앵커 검증

### Step 1 — Precheck (인벤토리 + 참조 스캔)

**1-1. Before 인벤토리 생성**:
```bash
bash docs/dev-plans/docs-renewal-tools/scan-inventory.sh before \
  > docs/dev-plans/docs-renewal-inventory-before.md
```
...
rg -n 'docs/plans|docs/dev-plans|docs/audit|docs/ERD|docs/HOME|docs/DESIGN_SYSTEM|docs/AI_SEARCH|docs/TEAM_WORKFLOW|docs/OBSIDIAN|docs/SETUP|견적서_VAT|DEPLOYMENT_GUIDE|DEVELOPMENT_GUIDELINES' \
  ...
  > docs/dev-plans/docs-renewal-refs-before.txt
```
...
rg -l 'status:\s*(approved|final)' docs/plans/ > /tmp/s_status.txt
# 본문 "사용자 최종승인 완료" 정확 매칭 + "구현 완료" (v2: 기획서 §2-4-A 정밀 일치)
rg -l '사용자 최종승인 완료' docs/plans/ | xargs rg -l '구현 완료' > /tmp/s_body.txt
# 로드맵
echo 'docs/plans/data-architecture-roadmap.md' > /tmp/s_roadmap.txt
# 통합 (중복 제거)
cat /tmp/s_suffix.txt /tmp/s_status.txt /tmp/s_body.txt /tmp/s_roadmap.txt | sort -u \
  > docs/dev-plans/docs-renewal-design-docs-list.txt
```
...
while IFS= read -r f; do
  # docs/plans/X.md 를 docs/product-specs/X.md → docs/design-docs/X.md 로 재이동
  new_base=$(basename "$f")
...
# 주: 실행 중 Step 1-3 리스트가 docs/dev-plans/ 에 있었으므로 exec-plans/ 로 이동됨
```
...
**4-1. Rename**:
```bash
git mv docs/DESIGN_SYSTEM.md docs/DESIGN.md
...
git mv DEPLOYMENT_GUIDE.md docs/exec-plans/archive/quotation-deploy.md
```
...
**삭제 대상** (통합·이관 완료 후):
- `DEVELOPMENT_GUIDELINES.md` (AGENTS.md 통합)
- `docs/HOME.md` (PRODUCT_SENSE 통합)
- `docs/TEAM_WORKFLOW.md` (AGENTS.md 통합)
- `DEPLOYMENT_GUIDE.md` 원본 (P4 에서 이관했으나 원본 제거 확인)

```bash
git rm DEVELOPMENT_GUIDELINES.md docs/HOME.md docs/TEAM_WORKFLOW.md
# DEPLOYMENT_GUIDE.md 는 P4 의 git mv 로 이미 삭제됨
...
### 이동 (git mv)
- `docs/plans/` → `docs/product-specs/` + 선별 `docs/design-docs/`
- `docs/dev-plans/` → `docs/exec-plans/`
- `docs/audit/` → `docs/generated/audit/`
- `docs/ERD.md` → `docs/generated/erd.md`
- `docs/견적서_VAT_표시기준.md` → `docs/product-specs/quotation-vat-rules.md` 등

### 축약
- `CLAUDE.md` (~50줄)

### 삭제
- `DEVELOPMENT_GUIDELINES.md` / `DEPLOYMENT_GUIDE.md` (원본 제거, 내용 이관)
- `docs/HOME.md` / `docs/TEAM_WORKFLOW.md`
