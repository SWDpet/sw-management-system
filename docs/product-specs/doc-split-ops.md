...
| FR-13 | 신규 4 종 폼 필드 (디자인팀 §C 추가 권고): **장애** (`severity P1/P2/P3 chip`, `downtime_minutes`, `customer_impact_level`, 발생일/증상/원인/조치) / **업무지원** (`request_channel 전화/이메일/현장`, `man_hour`, `satisfaction_score`, 요청자/지원내용) / **설치** (`pre_check_completed 체크박스`, `rollback_window_until`, `verification_evidence` 첨부, 환경/버전/검증) / **패치** (`patch_kind 보안/기능/긴급`, `affected_users_count`, **`rollback_plan` textarea 필수 — 미입력 시 저장 차단**, 대상/버전). 컬럼은 `OpsDocumentDetail.section_data jsonb` 에 저장 (전용 컬럼 추가 후속 스프린트). <br>※ 2026-06-16 [ops-time] 시/분 분리 변경: 장애 라벨 `장애시간(분)`→`장애처리시간`, 업무지원 `man_hour`(소수 시간) → **`man_minutes`(총 분)** 로 키 변경. 둘 다 시/분 분리 입력 후 총 분으로 저장. 기존 `man_hour` 데이터는 legacy(읽기 호환 미구현). <br>※ 2026-06-16 업무지원 `satisfaction_score`(만족도) 폼 필드 제거 — 작성자(엔지니어) 자기평가 모순(본인이 본인 만족도 평가). 소비처 없는 쓰기전용 키였음. 기존 데이터는 legacy. |
